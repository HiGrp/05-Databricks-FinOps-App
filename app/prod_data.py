...
def execute_sql(sql: str) -> tuple[pd.DataFrame | None, str | None]:
    ...
    With the "Demo data" source selected, queries run against a local DuckDB
...
def _resolve_warehouse_id(w) -> str | None:
    from connection import get_warehouse_id

    wh = get_warehouse_id()
    if wh:
        return wh
    for env_key in ("FINOPS_SQL_WAREHOUSE_ID",):
        env_wh = os.environ.get(env_key, "").strip()
        if env_wh:
            return env_wh
