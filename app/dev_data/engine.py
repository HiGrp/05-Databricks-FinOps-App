...
_DERIVED_VIEWS = {
    ...
    """,
    "billing_cost": """
        CREATE OR REPLACE VIEW billing_cost AS
        SELECT b.*,
               CAST(json_extract_string(p.pricing, '$.default') AS DOUBLE) AS list_price,
               b.usage_quantity * CAST(json_extract_string(p.pricing, '$.default') AS DOUBLE) AS list_cost
        FROM billing_usage_full b
        LEFT JOIN billing_list_prices p
