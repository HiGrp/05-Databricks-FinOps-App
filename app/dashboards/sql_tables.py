...
def _billing_cost(catalog: str) -> str:
    """billing_usage_full + list price valid at usage time (list_prices keeps price history)."""
    return f"""
SELECT
    b.*,
    CAST(COALESCE(p.pricing.effective_list.default, p.pricing.default) AS DOUBLE) AS list_price,
    b.usage_quantity
        * CAST(COALESCE(p.pricing.effective_list.default, p.pricing.default) AS DOUBLE) AS list_cost
FROM ({_billing_usage_full(catalog).strip()}) b
LEFT JOIN {catalog}.billing.list_prices p
    ON b.sku_name = p.sku_name
   AND b.cloud = p.cloud
   AND b.usage_unit = p.usage_unit
...
SELECT
    ...
    workspace_id, warehouse_type, auto_stop_minutes
FROM (
    SELECT
        warehouse_id, warehouse_name, warehouse_size, state,
        workspace_id, warehouse_type, auto_stop_minutes,
...
def _enriched_views(catalog: str) -> dict[str, str]:
    return {
        ...
        "billing_cost": _billing_cost(catalog),
